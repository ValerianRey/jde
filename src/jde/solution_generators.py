...
from jde.solutions import FullJDSolution, GradientDescentSolution, GramianJDSolution, Solution
...
def generate_gram_solutions(
    ...
    for _, (group_name, min_lr_exp, max_lr_exp) in lr_df.iterrows():
        gramian_weighting = KEY_TO_GRAMIAN_WEIGHTING[group_name]

        solution = GramianJDSolution(
            criterion=ssjd_criterion,
            train_batch_size=batch_size,
            evaluation_batch_size=batch_size,
            gramian_weighting=gramian_weighting,
            metrics=metrics,
            optimizer_class=optimizer_class,
            optimizer_kwargs=copy(optimizer_kwargs),
            architecture=architecture,
