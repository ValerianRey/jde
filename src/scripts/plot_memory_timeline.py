"""
Plots memory timeline evolution from profiling traces.
Usage:
  plot_memory_timeline <experiment> <folder>...

Arguments:
  <experiment>  Experiment filename stem (e.g., 'cifar10-bs32-cpu').
  <folder>      One or more method folders to compare (e.g., autograd autojac autogram).
...
def plot_memory_timelines(experiment: str, folders: list[str]) -> None:
    ...
    print(f"\n{'Folder':<20} {'Peak CPU Memory':>20} {'Peak CUDA Memory':>20}")
    print("-" * 62)
    for folder, cpu_timeline, cuda_timeline in zip(
        folders, cpu_timelines, cuda_timelines, strict=True
    ):
        peak_cpu = f"{int(cpu_timeline[:, 1].max()):,} B" if len(cpu_timeline) > 0 else "N/A"
        peak_cuda = f"{int(cuda_timeline[:, 1].max()):,} B" if len(cuda_timeline) > 0 else "N/A"
        print(f"{folder:<20} {peak_cpu:>20} {peak_cuda:>20}")
    print()
    ...
    output_dir = TRACES_DIR / "memory_timelines"
...
def main() -> None:
    args = docopt(__doc__)
    plot_memory_timelines(args["<experiment>"], args["<folder>"])
